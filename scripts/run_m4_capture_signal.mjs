...
async function sendRaw(connection, prepared) {
  const returned = await connection.sendRawTransaction(prepared.bytes, { skipPreflight: true, maxRetries: 0 });
...
async function createSignatureWatch(connection, signature, commitment, onSuccess = null) {
  ...
  // Attach a handler immediately so a fast rejected notification can never become an
  // unhandled rejection before the benchmark reaches the corresponding await.
  void promise.catch(() => {});
...
async function createFixture({ baseProgram, baseProvider, baseConnection, providerPayer, mode, scenarioSeed }) {
  ...
  const objectiveSeed = crypto.createHash('sha256').update(`reactor-m4-signal:${scenarioSeed}:${mode}:${crypto.randomBytes(8).toString('hex')}`).digest();
  ...
  const decisionBuilder = mode === 'magicblock'
    ? activeProgram.methods.evaluateSessionCandidate(EXPECTED_SEQUENCES.map((number) => new anchor.BN(number))).accounts({
    ...
    : activeProgram.methods.evaluateAndLock(
        EXPECTED_SEQUENCES.map((number) => new anchor.BN(number)),
...
async function runTrial({ mode, windowMs, trialIndex, baseProgram, baseProvider, baseConnection, providerPayer }) {
  ...
  const telemetry = new TrialTelemetry({
    scenarioId: `m4-signal-${mode}-${seed}`,
    path: mode,
  ...
  telemetry.config({
    ...
    openingObservation: 'condition-account-subscription-processed',
    captureObservation: 'prewarmed-decision-signature-subscription-processed',
    exactVersionVerification: 'post-capture-account-read',
    durabilityRequirement: 'prewarmed-confirmed-signature-subscriptions',
  });
  ...
  let openingResolve;
  let openingReject;
  const openingPromise = new Promise((resolve, reject) => { openingResolve = resolve; openingReject = reject; });
    ...
    (accountInfo, context) => {
      ...
      try {
        ...
        if (state.sequence === 2 && state.predicateResult === true) {
          ...
          openingResolve({ state, slot: context.slot });
        }
      } catch (error) {
        openingResolved = true;
        openingReject(error);
  ...
  // All three transaction signatures are known before T0, so their watches can be
  // active before the opening event. This removes signature-subscription setup from
  // the measured path and guarantees decision failures surface instead of timing out
  // behind a nonexistent lock account.
  const openConfirmedWatch = await createSignatureWatch(
    fixture.activeConnection,
    fixture.openPrepared.signature,
    'confirmed',
  );
  const decisionProcessedWatch = await createSignatureWatch(
    fixture.activeConnection,
    fixture.decisionPrepared.signature,
    'processed',
    (context) => telemetry.mark('capture_observed', {
      slot: context.slot,
      evidence: 'decision-signature-processed',
    }),
  );
  const decisionConfirmedWatch = await createSignatureWatch(
    fixture.activeConnection,
    fixture.decisionPrepared.signature,
    'confirmed',
  );
  ...
  try {
    ...
    await withTimeout(openingPromise, 'opening condition subscription');
    ...
    await withTimeout(decisionProcessedWatch.promise, 'decision processed signature');
    captureStateObserved = true;

    // Verify exact frozen versions after the capture timestamp. This read cannot make
    // a slow path look faster, but it can invalidate an apparently successful lock.
    if (mode === 'magicblock') {
  ...
  try {
    if (openSignature) {
      await withTimeout(openConfirmedWatch.promise, 'open confirmed', Math.max(OBSERVATION_TIMEOUT_MS, 10_000));
      openConfirmed = true;
      telemetry.mark('window_open_confirmed');
    }
  } catch (error) {
    telemetry.mark('window_open_confirmation_failed', { error: String(error?.message ?? error) });
    if (!decisionError) decisionError = String(error?.message ?? error);
  }
  try {
    if (decisionSignature) {
      await withTimeout(decisionConfirmedWatch.promise, 'decision confirmed', Math.max(OBSERVATION_TIMEOUT_MS, 10_000));
  ...
  try {
    if (closeSignature) {
      await withTimeout(closeConfirmedWatch.promise, 'close confirmed', Math.max(OBSERVATION_TIMEOUT_MS, 10_000));
      closeConfirmed = true;
  ...
  await Promise.allSettled([
    ...
    fixture.activeConnection.removeSignatureListener(openConfirmedWatch.subscriptionId),
    fixture.activeConnection.removeSignatureListener(decisionProcessedWatch.subscriptionId),
    fixture.activeConnection.removeSignatureListener(decisionConfirmedWatch.subscriptionId),
    fixture.activeConnection.removeSignatureListener(closeConfirmedWatch.subscriptionId),
  ]);
...
console.log('M4a SIGNAL capture benchmark');
...
console.log('measured path: send signed open → condition account subscription → send signed decision → pre-warmed decision signature subscription');
console.log('exact lock/candidate versions are verified after the capture timestamp; confirmation uses pre-warmed signature subscriptions');
...
const output = {
  benchmark: 'reactor-m4a-signal-capture',
  scope: 'prebuilt-warmed-event-driven-hot-path-capture-mechanics-not-production-performance',
