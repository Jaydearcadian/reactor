...
BASE_RPC="${REACTOR_M5_BASE_RPC:-http://127.0.0.1:8899}"
BASE_WS="${REACTOR_M5_BASE_WS:-ws://127.0.0.1:8900}"
ER_RPC="${REACTOR_M5_ER_RPC:-http://127.0.0.1:7799}"
ER_WS="${REACTOR_M5_ER_WS:-ws://127.0.0.1:7800}"
ER_VALIDATOR="${REACTOR_M5_ER_VALIDATOR:-mAGicPQYBMvcYveUZA5F5UNNwyHvfYh5xkLS2Fr1mev}"
...
LOG_DIR="experiment/results/m5-coordination-efficiency-logs"
...
solana airdrop 1000 "$PAYER" --url "$BASE_RPC" >/dev/null
...
echo "Installing dependencies and deriving local rent floor..."
npm install
...
if [[ ! "$RENT_FLOOR_LAMPORTS" =~ ^[0-9]+$ ]]; then
  echo "Could not determine zero-data rent floor: $RENT_FLOOR_LAMPORTS" >&2
  exit 1
fi
export REACTOR_M5_ATTEMPT_PAYER_LAMPORTS="$((RENT_FLOOR_LAMPORTS + 100000))"
export REACTOR_M5_SOURCE_PAYER_LAMPORTS="$((RENT_FLOOR_LAMPORTS + 250000))"
echo "Zero-data rent floor:       $RENT_FLOOR_LAMPORTS lamports"
echo "Coordinator payer funding:  $REACTOR_M5_ATTEMPT_PAYER_LAMPORTS lamports/account"
echo "Source payer funding:       $REACTOR_M5_SOURCE_PAYER_LAMPORTS lamports/account"
...
export REACTOR_M5_BASE_RPC="$BASE_RPC"
export REACTOR_M5_BASE_WS="$BASE_WS"
export REACTOR_M5_ER_RPC="$ER_RPC"
export REACTOR_M5_ER_WS="$ER_WS"
export REACTOR_M5_ER_VALIDATOR="$ER_VALIDATOR"
export REACTOR_M5_OBJECTIVE_COUNTS="${REACTOR_M5_OBJECTIVE_COUNTS:-1,5,10}"
export REACTOR_M5_WINDOW_MS="${REACTOR_M5_WINDOW_MS:-100}"
export REACTOR_M5_JITTER_STEP_MS="${REACTOR_M5_JITTER_STEP_MS:-7}"
export REACTOR_M5_SPEC_CADENCE_MS="${REACTOR_M5_SPEC_CADENCE_MS:-5}"
export REACTOR_M5_PREOPEN_MS="${REACTOR_M5_PREOPEN_MS:-25}"
export REACTOR_M5_POSTCLOSE_MS="${REACTOR_M5_POSTCLOSE_MS:-25}"

echo
echo "Running M5 Coordination Efficiency smoke..."
echo "objective counts=$REACTOR_M5_OBJECTIVE_COUNTS"
echo "source spacing=${REACTOR_M5_WINDOW_MS}ms jitter=${REACTOR_M5_JITTER_STEP_MS}ms"
echo "solana speculation cadence=${REACTOR_M5_SPEC_CADENCE_MS}ms/objective"
node scripts/run_m5_coordination_efficiency_local.mjs

echo
echo "Evidence: experiment/results/m5-coordination-efficiency-local-latest.json"
