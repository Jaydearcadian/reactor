...
const DURABILITY_TIMEOUT_MS = Number(process.env.REACTOR_M4_DURABILITY_TIMEOUT_MS ?? 10000);
...
async function removePendingSignatureWatch(connection, watch) {
  ...
  try {
    ...
    // Best-effort cleanup only. A one-shot signature subscription may have been
    // removed by the RPC between the settled check and this call.
  }
}
...
async function createFixture({ baseProgram, baseProvider, baseConnection, providerPayer, mode, scenarioSeed }) {
  ...
  const objectiveSeed = crypto
    .createHash('sha256')
    .update(`reactor-m4-signature:${scenarioSeed}:${mode}:${crypto.randomBytes(8).toString('hex')}`)
    .digest();
  ...
  const decisionBuilder = mode === 'magicblock'
    ? activeProgram.methods.evaluateSessionCandidate(
...
async function runTrial({ mode, windowMs, trialIndex, baseProgram, baseProvider, baseConnection, providerPayer }) {
  ...
  const telemetry = new TrialTelemetry({
    scenarioId: `m4-signature-${mode}-${seed}`,
    path: mode,
  ...
  telemetry.config({
    ...
    openingObservation: 'prewarmed-opening-signature-subscription-processed',
  ...
  const openProcessedWatch = await createSignatureWatch(
    ...
    'processed',
    (context) => telemetry.mark('condition_observed', {
      slot: context.slot,
      evidence: 'opening-signature-processed',
    }),
  );
  ...
  try {
    ...
    await withTimeout(
      openProcessedWatch.promise,
      'opening transaction processed',
    ...
    await withTimeout(
      decisionProcessedWatch.promise,
      'decision transaction processed',
      OBSERVATION_TIMEOUT_MS,
    );
    ...
    try {
      if (mode === 'magicblock') {
        ...
        const lock = await fixture.activeProgram.account.executionLock.fetch(
          fixture.lockPda,
          'processed',
        );
      ...
      exactVerificationError = String(error?.message ?? error);
    }
  } catch (error) {
  ...
  try {
    if (openSignature) {
      await withTimeout(openConfirmedWatch.promise, 'open confirmed', DURABILITY_TIMEOUT_MS);
      openConfirmed = true;
  ...
  try {
    if (decisionSignature) {
      await withTimeout(decisionConfirmedWatch.promise, 'decision confirmed', DURABILITY_TIMEOUT_MS);
  ...
  try {
    if (closeSignature) {
      await withTimeout(closeConfirmedWatch.promise, 'close confirmed', DURABILITY_TIMEOUT_MS);
      closeConfirmed = true;
  ...
  await Promise.allSettled([
    removePendingSignatureWatch(fixture.activeConnection, openProcessedWatch),
    removePendingSignatureWatch(fixture.activeConnection, openConfirmedWatch),
    removePendingSignatureWatch(fixture.activeConnection, decisionProcessedWatch),
    removePendingSignatureWatch(fixture.activeConnection, decisionConfirmedWatch),
    removePendingSignatureWatch(fixture.activeConnection, closeConfirmedWatch),
  ]);

  const verificationAmbiguous = exactVerificationError != null;
  ...
  telemetry.set({
    ...
    falseLock: captureStateObserved && !verificationAmbiguous && !exactVersionMatch,
    ambiguous,
    openConfirmed,
    decisionConfirmed,
    closeConfirmed,
...
console.log('M4a SIGNATURE capture benchmark');
...
console.log('measured path: send signed open → pre-warmed opening signature processed → send signed decision → pre-warmed decision signature processed');
  ...
  for (let trialIndex = 0; trialIndex < TRIALS_PER_WINDOW; trialIndex += 1) {
    for (const mode of modes) {
      ...
      const vector = trial.diagnosticConditionVector
        ...
        .join(',') ?? 'n/a';
      console.log(
        `${mode} window=${windowMs}ms capture=${trial.capture} exact=${trial.exactVersionMatch}`
        + ` stale=${trial.staleAttempt} falseLock=${trial.falseLock} ambiguous=${trial.ambiguous}`
        + ` latency=${trial.latency.captureMs?.toFixed(2) ?? 'n/a'}ms vector=[${vector}]`
        + failureText
        + verificationText,
      );
    }
...
const output = {
  benchmark: 'reactor-m4a-signature-capture',
  scope: 'prebuilt-warmed-signature-driven-hot-path-capture-mechanics-not-production-performance',
