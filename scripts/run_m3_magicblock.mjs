...
const BASE_RPC = process.env.REACTOR_BASE_RPC ?? "https://api.devnet.solana.com";
const ER_RPC = process.env.REACTOR_ER_RPC ?? "https://devnet.magicblock.app/";
const ER_WS = process.env.REACTOR_ER_WS ?? "wss://devnet.magicblock.app/";
...
async function send(builder, signers = []) {
  try {
    const signed = signers.length > 0 ? builder.signers(signers) : builder;
    return await signed.rpc({ commitment: "confirmed" });
  } finally {
    await pace();
  }
}

async function waitForAccount(connection, pubkey, predicate, label, attempts = 120) {
  for (let i = 0; i < attempts; i += 1) {
    const info = await connection.getAccountInfo(pubkey, "confirmed");
    if (info && predicate(info)) return info;
    await sleep(250);
  }
  throw new Error(`timed out waiting for ${label}: ${pubkey.toBase58()}`);
}

async function waitForSignature(connection, signature, attempts = 120) {
  for (let i = 0; i < attempts; i += 1) {
...
if (!fs.existsSync(idlPath)) {
  throw new Error(`missing ${idlPath}; run anchor build before M3`);
}
...
await baseProvider.sendAndConfirm(
  new Transaction().add(
    SystemProgram.transfer({
      ...
      toPubkey: authority,
      lamports: AUTHORITY_BUDGET_LAMPORTS,
    }),
    SystemProgram.transfer({
      fromPubkey: providerPayer,
      toPubkey: recipient,
...
for (let kind = 0; kind < CONDITION_COUNT; kind += 1) {
  await waitForAccount(
    erConnection,
    conditionPdas[kind],
    (info) => info.owner.equals(programId),
    `ER condition ${kind}`,
  );
}
await waitForAccount(erConnection, candidatePda, (info) => info.owner.equals(programId), "ER session candidate");

console.log("delegation visible in ER");
...
for (let kind = 0; kind < CONDITION_COUNT; kind += 1) {
  await updateEr(kind, 1, 100 + kind, true);
}
...
await expectFailure("ER stale exact sequence", () =>
  send(
    erProgram.methods
      .evaluateSessionCandidate([1, 1, 1, 1, 1, 1].map((n) => new anchor.BN(n)))
...
await expectFailure("ER false predicate", () =>
  send(
    erProgram.methods
      .evaluateSessionCandidate([1, 1, 2, 1, 1, 1].map((n) => new anchor.BN(n)))
      .accounts(evaluationAccounts),
  ),
);
...
const sealSignature = await send(
  erProgram.methods
    .evaluateSessionCandidate(frozenSequences.map((n) => new anchor.BN(n)))
    .accounts(evaluationAccounts),
);
...
assert(
  candidateBeforeMutation.frozenSequences.map(Number).join(",") === frozenSequences.join(","),
  "ER candidate did not freeze exact sequences",
);
...
assert(
  candidateAfterMutation.frozenSequences.map(Number).join(",") === frozenSequences.join(","),
  "post-seal condition update mutated candidate",
);

const finalizeSignature = await send(
  erProgram.methods
    .finalizeSessionCandidate()
    .accounts({ payer: authority, sessionCandidate: candidatePda }),
  [authorityKeypair],
);

await waitForSignature(erConnection, finalizeSignature);
await waitForAccount(
  baseConnection,
  candidatePda,
  (info) => info.owner.equals(programId),
  "base-layer undelegated session candidate",
);
...
assert(
  committedCandidate.frozenSequences.map(Number).join(",") === frozenSequences.join(","),
  "committed candidate sequence evidence changed",
);
...
assert(
  materializedLock.sequences.map(Number).join(",") === frozenSequences.join(","),
  "base ExecutionLock does not match ER-sealed candidate",
);
...
const result = {
  ...
  finalizeAndUndelegateSignature: finalizeSignature,
  candidateObservedBackOnSolana: true,
