...
async function fundFixture(baseProvider, providerPayer, authority) {
  await baseProvider.sendAndConfirm(
    new Transaction().add(SystemProgram.transfer({ fromPubkey: providerPayer, toPubkey: authority, lamports: FIXTURE_BUDGET_LAMPORTS })),
    [],
  );
  if (SETUP_PACE_MS > 0) await sleep(SETUP_PACE_MS);
}
...
async function createFixture({ baseProgram, baseProvider, baseConnection, providerPayer, mode, scenarioSeed }) {
  ...
  if (mode === 'magicblock') {
    ...
    const erProvider = new anchor.AnchorProvider(erConnection, baseProvider.wallet, { commitment: 'confirmed', preflightCommitment: 'confirmed' });
  ...
  async function update(kind, sequence, predicateResult, measured = false) {
    const builder = activeProgram.methods.updateCondition(new anchor.BN(sequence), new anchor.BN(100 + kind), predicateResult, new anchor.BN(validityUntilSlot)).accounts({ condition: conditionPdas[kind], source: sources[kind].publicKey });
    return measured ? measuredSend(builder, [sources[kind]]) : setupSend(builder, [sources[kind]]);
  }

  for (let kind = 0; kind < CONDITION_COUNT; kind += 1) await update(kind, 1, kind !== 2, false);

  return { authorityKeypair, authority, pathPda, recipient, objectivePda, vaultPda, conditionPdas, candidatePda, lockPda, activeConnection, activeProgram, update, validator, erEndpoint, validityAnchorSlot, validityUntilSlot };
}
...
async function runTrial({ mode, windowMs, trialIndex, baseProgram, baseProvider, baseConnection, providerPayer }) {
  ...
  const telemetry = new TrialTelemetry({ scenarioId, path: mode, cluster: 'solana-devnet', windowMs, seed, expectedSequences: EXPECTED_SEQUENCES });
  telemetry.config({
    programId: baseProgram.programId.toBase58(), baseRpc: BASE_RPC, magicBlockBaseRpc: MAGICBLOCK_BASE_RPC, routerRpc: ROUTER_RPC,
    erEndpoint: fixture.erEndpoint, validator: fixture.validator, setupExcludedFromTiming: true, conditionTtlSlots: CONDITION_TTL_SLOTS,
    validityAnchorSlot: fixture.validityAnchorSlot, validityUntilSlot: fixture.validityUntilSlot,
    authorityModel: 'independent-source-transactions', measuredPathHasNoSlotLookup: true,
    captureCommitment: 'processed', durabilityCommitment: 'confirmed',
  });
  ...
  try {
    ...
    if (mode === 'magicblock') {
      const evaluationAccounts = {
        ...
        condition0: fixture.conditionPdas[0], condition1: fixture.conditionPdas[1], condition2: fixture.conditionPdas[2],
        condition3: fixture.conditionPdas[3], condition4: fixture.conditionPdas[4], condition5: fixture.conditionPdas[5],
      };
      decisionSignature = await measuredSend(fixture.activeProgram.methods.evaluateSessionCandidate(EXPECTED_SEQUENCES.map((n) => new anchor.BN(n))).accounts(evaluationAccounts));
    } else {
      decisionSignature = await measuredSend(
        fixture.activeProgram.methods.evaluateAndLock(EXPECTED_SEQUENCES.map((n) => new anchor.BN(n)), new anchor.BN(TRANSFER_LAMPORTS), new anchor.BN(EXPOSURE_REDUCTION)).accounts({
          payer: fixture.authority, path: fixture.pathPda, objective: fixture.objectivePda, vault: fixture.vaultPda, recipient: fixture.recipient,
          condition0: fixture.conditionPdas[0], condition1: fixture.conditionPdas[1], condition2: fixture.conditionPdas[2],
          condition3: fixture.conditionPdas[3], condition4: fixture.conditionPdas[4], condition5: fixture.conditionPdas[5],
          executionLock: fixture.lockPda, systemProgram: SystemProgram.programId,
    ...
    telemetry.mark('capture_observed', { commitment: 'processed', captureStateObserved });
    ...
    telemetry.set({ capture: false, captureStateObserved: false, falseLock: false, failure: decisionError });
  }
  ...
  const ambiguous = Boolean((openSignature && !openConfirmed) || (decisionSignature && !decisionConfirmed) || !closeConfirmed);

  telemetry.set({ capture: durableCapture, ambiguous, openConfirmed, decisionConfirmed, closeConfirmed, failure: decisionError });
...
for (const windowMs of WINDOW_MS) {
  for (let trialIndex = 0; trialIndex < TRIALS_PER_WINDOW; trialIndex += 1) {
    for (const mode of modes) {
      ...
      console.log(`${mode} window=${windowMs}ms capture=${trial.capture} exact=${trial.exactVersionMatch} stale=${trial.staleAttempt} falseLock=${trial.falseLock} ambiguous=${trial.ambiguous} latency=${trial.latency.captureMs?.toFixed(2) ?? 'n/a'}ms${failure}`);
    }
  }
}
...
const output = {
  ...
  configuration: {
    baseRpc: BASE_RPC, magicBlockBaseRpc: MAGICBLOCK_BASE_RPC, routerRpc: ROUTER_RPC, windowsMs: WINDOW_MS,
    trialsPerWindowPerPath: TRIALS_PER_WINDOW, fixtureBudgetLamports: FIXTURE_BUDGET_LAMPORTS, setupPaceMs: SETUP_PACE_MS,
    conditionTtlSlots: CONDITION_TTL_SLOTS, measuredPathHasNoSlotLookup: true, captureCommitment: 'processed', durabilityCommitment: 'confirmed',
  },
