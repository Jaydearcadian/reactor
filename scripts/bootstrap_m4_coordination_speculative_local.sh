...
LOG_DIR="experiment/results/m4-speculative-logs"
...
echo
echo "Running M4-Coordination speculative adversarial smoke..."
echo "windows=${REACTOR_M4_SPEC_WINDOWS_MS:-10,20,50,100,150,250}"
echo "trials/band/path=${REACTOR_M4_SPEC_TRIALS:-2}"
echo "preopen=${REACTOR_M4_SPEC_PREOPEN_MS:-25}ms cadence=${REACTOR_M4_SPEC_CADENCE_MS:-5}ms postclose=${REACTOR_M4_SPEC_POSTCLOSE_MS:-25}ms"
node scripts/run_m4_coordination_speculative_local.mjs

echo
echo "Evidence: experiment/results/m4-coordination-speculative-local-latest.json"
